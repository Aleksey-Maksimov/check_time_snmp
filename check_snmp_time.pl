#!/usr/bin/perl
#########################################################################################
# check_time_snmp.pl v3.5 (2025-12-06)
#
# Purpose:
...
#   v3.4 (2025-12-01) - Correct parsing when --time-format is used together with --timezone:
#                       create a DateTime from parsed components so the device's timezone
#                       is applied correctly (was incorrectly reinterpreting epoch).
#                       - Avoid double/misapplied timezone conversion: timezone conversion
#                         block now only runs when time was parsed via the "fallback" branch.
#                       - Support offset timezone strings like +0300 as +03:00 for DateTime::TimeZone.
#   v3.5 (2025-12-06) - Added --time-preformat-regex option:
#                       allows applying a Perl substitution (s/.../.../flags) to the
#                       SNMP-returned time string before it's parsed by --time-format.
#                       Keeps existing parsing flow unchanged for other devices.
#
# Usage Examples:
#   Basic SNMPv2c check:
...
my ( $opt_host, $opt_port, $opt_community, $opt_warning, $opt_critical, $opt_help, $opt_version,
     $opt_oid, $opt_time_format, $opt_timezone, $opt_ntp_server, $opt_ntp_port, $opt_protocol,
     $opt_username, $opt_authpassword, $opt_authprotocol, $opt_privpassword, $opt_privprotocol,
     $opt_verbose, $opt_time_preformat_regex );
...
GetOptions(
    ...
    'time-preformat-regex=s' => \$opt_time_preformat_regex,
...
if ($opt_version) {
    print "check_time_snmp.pl v3.5 (2025-12-06)\n";
...
if ($snmp_output =~ /=\s+\S+:\s+(.*)$/) {
    ...
    # --- NEW: apply user-provided preformat regex substitution (Perl s///) ---
    if (defined $opt_time_preformat_regex && length $opt_time_preformat_regex) {
        my $expr = $opt_time_preformat_regex;
        # only accept a substitution-like expression starting with s and proper delimiters
        unless ($expr =~ /^\s*s(.).*\1.*\1[imsx]*\s*$/s) {
            print "UNKNOWN: --time-preformat-regex must be a Perl substitution expression like s/pat/repl/flags\n";
            exit $ERRORS{'UNKNOWN'};
        }
        # perform substitution in a controlled eval
        my $code = "\$device_time_str =~ $expr;";
        eval $code;
        if ($@) {
            print "UNKNOWN: --time-preformat-regex evaluation failed: $@\n";
            exit $ERRORS{'UNKNOWN'};
        }
        # Trim again (in case substitution introduced extra spaces)
        $device_time_str =~ s/^\s+|\s+$//g;
    }
    # --- END NEW ---
} else {
...
# use rounded value (3 decimals) in perfdata
my $diff_perf = sprintf("%.3f", $diff);
print "[$status] $status_message | 'time.offset.seconds'=$diff_perf;$warn_threshold;$crit_threshold;;\n";
...
sub print_help {
    print <<'EOT';
Usage: $0 -H <host> -w <warning> -c <critical> [options]
